...
class RecipeForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'instructions': forms.Textarea(attrs={'rows': 4, 'maxlength': 2000, 'class': 'textarea'}),
            'cooking_time': forms.NumberInput(attrs={'min': 1, 'class': 'input'})
        }
    ...
    def clean(self):
        ...
        for form in self.forms:
            ...
            if form.cleaned_data.get('DELETE', False):
                continue
            if not form.cleaned_data.get('ingredient') or not form.cleaned_data.get('quantity'):
                continue
            if form.cleaned_data.get('ingredient') in ingredients:
                raise forms.ValidationError("You can't add the same ingredient again")
